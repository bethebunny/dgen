...
- Origin destructors beyond the base case. ``LowerDestroy`` frees heap destroys whose origin thread resolves statically to the allocation (stores, chains, unpacks). Origins from function parameters or branch results raise ``UnresolvableOriginError`` and await the ``attach``/destructor-stack design in `docs/origins.md`. Stack destroys keep no runtime action, so LLVM lifetime markers are also future work.
