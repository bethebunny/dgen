...
- Extend the linearity contract registry (``_BLOCK_CONTRACTS`` in `dgen/ir/verification.py`; contracts landed for ``unpack``/``if``/``try``): loops need the carry-pair rule first (see loop-carry bullet), the goto family stays unregistered (label bodies run zero-or-more times), and contracts should eventually be declarable in ``.dgen`` op definitions rather than a Python-side registry.
