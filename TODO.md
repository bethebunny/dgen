...
- Extend aggregate taint beyond products. Tuple and Array multiplicity and handler containment are hereditary now (`component_types` in `dgen/builtins.py`). Sums and existentials still need the may-contain rule, and user-declared record types with substructural data fields are not yet joined. Heredity stops at pointer indirection, which is the escape-analysis question in `docs/origins.md`.
