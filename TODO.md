...
- Origin base destructors. ``memory.destroy`` currently lowers to a no-op (same leak the old ``deallocate`` had); real ``free`` for heap origins arrives with the ``attach``/destructor-stack design in `docs/origins.md`.
