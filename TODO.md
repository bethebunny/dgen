...
- Loop carries under ``ZeroOrMore``: ``for``/``while`` now reject linear captures outright (sound but restrictive) — supporting linear loop-carried values needs the carry-pair rule (see loop-carry bullet), which upgrades the ``ZeroOrMore`` contract from rejection to yield-as-consume verification. The goto family stays trait-less by design: label multiplicity is a property of the branch graph, not the op; if goto-level precision is ever needed, it takes per-label multiplicity annotations stamped by lowerings or a CFG dataflow analysis.
