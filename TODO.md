...
- Declare block-execution traits on more ops (``ExactlyOnce``/``Alternatives``/``BodyWithHandler`` in `dgen/dialects/builtin.dgen`, dispatched through ``Op.verify_block_linearity``; declared for ``unpack``/``if``/``try``): loops need the carry-pair rule first (see loop-carry bullet), and the goto family declares none because label bodies run zero-or-more times.
