...
- Loop carries under ``ZeroOrMore``. Loops and function bodies now reject linear captures outright, which is sound but restrictive. Supporting linear loop-carried values needs the carry-pair rule (see the loop-carry bullet), which upgrades the ``ZeroOrMore`` contract from rejection to yield-as-consume verification. The goto family declares ``ExtrinsicBlocks`` by design, since label multiplicity is a property of the branch graph rather than the op. If goto-level precision is ever needed, it takes per-label multiplicity annotations stamped by lowerings or a CFG dataflow analysis.
...
- Aggregate linearity taint. ``Tuple<[Ref<T>, Origin]>`` (an allocation result) is not itself ``Linear``, so unpacking the same allocation tuple twice would silently duplicate the origin. The rule (see the "handlers inside aggregate types" open question in `docs/origins.md`): an aggregate with a linear component is linear, and a `Handler<Diverge>` component taints the aggregate for `Value.totality`.
- Destructors beyond the attach base case. ``memory.attach`` puts a destructor function value in the origin's type and ``LowerDestroy`` applies it (inline splice for literals, indirect call otherwise). Remaining gaps: a destructor cannot clean up other linear values because function bodies reject linear captures (the destructor-stack forcing point in `docs/origins.md`), a shared destructor literal raises ``SharedDestructorError`` until a block copy utility lands, and stack destroys keep no runtime action so LLVM lifetime markers are future work.
- Operand-dependent result types in the spec language. The attach design wants ``memory.store`` to declare that its result type equals its origin operand's type, so a destructor-carrying ``Origin`` parameter survives threading. The `.dgen` grammar has no way to write this. Until it does, ops declare the bare type and the memory lowering pass verifies pre-lowering that each origin-threading op's annotated result type matches its operand's type.
