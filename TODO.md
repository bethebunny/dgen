...
- Support blocks whose result is "totally terminating" (e.g. body of a try where every path raises): a wrapping `branch<%exit>([never_value])` should emit cleanly, not produce a phi referencing an undefined Nil-typed value. Today `RaiseCatchToGoto.lower_try` and `ControlFlowToGoto._make_branch_label` skip the wrapping branch when `block.result.type is Never` to work around this. Once codegen handles wrapping branches over already-terminating bodies, both can drop the `Never` check.
- Principled divergence detection: the `Never`-type check used in goto lowerings is a proxy for "this op already terminates" — it works because `Never` is currently only reached by `RaiseOp` (which lowers to a `BranchOp`). When a real "does this op diverge?" analysis lands, the lowerings should use it instead of `isinstance(_, Never)`. Same proxy is in `ControlFlowToGoto._make_branch_label` and `RaiseCatchToGoto.lower_try`.
