...
class AlgebraToLLVM(Pass):
    ...
    def lower_add(self, op: algebra.AddOp, rewriter: Rewriter) -> bool:
        if isinstance(op.type, F64):
            rewriter.replace_uses(op, llvm.FaddOp(lhs=op.left, rhs=op.right))
        else:
            rewriter.replace_uses(op, llvm.AddOp(lhs=op.left, rhs=op.right))
        return True

    @lowering_for(algebra.MultiplyOp)
    def lower_multiply(self, op: algebra.MultiplyOp, rewriter: Rewriter) -> bool:
        if isinstance(op.type, F64):
            rewriter.replace_uses(op, llvm.FmulOp(lhs=op.left, rhs=op.right))
        else:
            rewriter.replace_uses(op, llvm.MulOp(lhs=op.left, rhs=op.right))
        return True
