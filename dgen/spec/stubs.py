...
def _trait_stub(name: str, cls: type, type_to_name: dict[type, str]) -> Iterator[str]:
    ...
    Unparameterized traits: simple ``class Foo(Trait): ...``. Parametric
    traits (``__params__`` set) emit as dataclasses with one field per
    param — same shape as a parametric type — so users can construct
    ``Handler(effect_type=...)`` and compare structurally in has_trait.
    """
...
def generate_pyi(module: ModuleType, dialect_name: str) -> str:
    ...
    has_non_trait_types = any(
        not issubclass(cls, Trait) for cls in dialect.types.values()
    )
    ...
    # Emit the class bodies first so we can tell which `from X import ...`
    # names actually appear in the stub. Avoids dragging in every import
    # the .dgen file declared whether or not this dialect's classes use it.
    body_lines: list[str] = []
