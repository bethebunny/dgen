...
def test_full_pipeline(ir_snapshot):
    ...
    ir_text = strip_prefix("""
        ...
        |     %3 : toy.Tensor<ndbuffer.Shape<2>([2, 3]), number.Float64> = toy.reshape(%2)
        |     %4 : toy.Tensor<ndbuffer.Shape<2>([3, 2]), number.Float64> = toy.transpose(%1)
        |     %5 : toy.Tensor<ndbuffer.Shape<2>([3, 2]), number.Float64> = toy.transpose(%3)
        |     %6 : toy.Tensor<ndbuffer.Shape<2>([3, 2]), number.Float64> = toy.mul(%4, %5)
        |     %7 : toy.Tensor<ndbuffer.Shape<2>([3, 2]), number.Float64> = toy.transpose(%3)
        |     %8 : toy.Tensor<ndbuffer.Shape<2>([3, 2]), number.Float64> = toy.transpose(%1)
        |     %9 : toy.Tensor<ndbuffer.Shape<2>([3, 2]), number.Float64> = toy.mul(%7, %8)
        |     %10 : Nil = toy.print(%9)
        |     %_ : Nil = chain(%10, %6)
    """)
    m = asm.parse(ir_text)
