...
def test_roundtrip_tile():
    ir = strip_prefix("""
        ...
        | import index
        |
        | %f : function.Function<toy.Tensor<memory.Shape<2>([4, 3]), F64>> = function.function<toy.Tensor<memory.Shape<2>([4, 3]), F64>>() body(%a: toy.Tensor<memory.Shape<1>([3]), F64>, %n: index.Index):
