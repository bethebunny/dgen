...
def test_transpose():
    ...
    expected = strip_prefix("""
        ...
        |     %5 : () = llvm.phi([%2, %33], ["entry", "loop_exit1"])
        |     %6 : index = 2
        |     %7 : () = llvm.icmp("slt", %5, %6)
        |     %8 : () = llvm.cond_br(%7, "loop_body0", "loop_exit0")
        |     %9 : () = llvm.label("loop_body0")
        |     %10 : index = 0
        |     %11 : () = llvm.br("loop_header1")
        |     %12 : () = llvm.label("loop_header1")
        |     %13 : () = llvm.phi([%10, %29], ["loop_body0", "loop_body1"])
        |     %14 : index = 3
        |     %15 : () = llvm.icmp("slt", %13, %14)
        |     %16 : () = llvm.cond_br(%15, "loop_body1", "loop_exit1")
...
def test_element_wise_mul():
    ...
    expected = strip_prefix("""
        ...
        |     %6 : () = llvm.phi([%3, %40], ["entry", "loop_exit1"])
        |     %7 : index = 2
        |     %8 : () = llvm.icmp("slt", %6, %7)
        |     %9 : () = llvm.cond_br(%8, "loop_body0", "loop_exit0")
        |     %10 : () = llvm.label("loop_body0")
        |     %11 : index = 0
        |     %12 : () = llvm.br("loop_header1")
        |     %13 : () = llvm.label("loop_header1")
        |     %14 : () = llvm.phi([%11, %36], ["loop_body0", "loop_body1"])
        |     %15 : index = 2
        |     %16 : () = llvm.icmp("slt", %14, %15)
        |     %17 : () = llvm.cond_br(%16, "loop_body1", "loop_exit1")
...
def test_3d_element_wise_add():
    ...
    expected = strip_prefix("""
        ...
        |     %4 : () = llvm.br("loop_header0")
        |     %5 : () = llvm.label("loop_header0")
        |     %6 : () = llvm.phi([%3, %61], ["entry", "loop_exit1"])
        |     %7 : index = 2
        |     %8 : () = llvm.icmp("slt", %6, %7)
        |     %9 : () = llvm.cond_br(%8, "loop_body0", "loop_exit0")
        |     %10 : () = llvm.label("loop_body0")
        ...
        |     %12 : () = llvm.br("loop_header1")
        |     %13 : () = llvm.label("loop_header1")
        |     %14 : () = llvm.phi([%11, %57], ["loop_body0", "loop_exit2"])
        |     %15 : index = 2
        |     %16 : () = llvm.icmp("slt", %14, %15)
        |     %17 : () = llvm.cond_br(%16, "loop_body1", "loop_exit1")
        |     %18 : () = llvm.label("loop_body1")
        ...
        |     %22 : () = llvm.phi([%19, %53], ["loop_body1", "loop_body2"])
        |     %23 : index = 2
        |     %24 : () = llvm.icmp("slt", %22, %23)
        |     %25 : () = llvm.cond_br(%24, "loop_body2", "loop_exit2")
...
def test_multiply_transpose_inlined():
    ...
    expected = strip_prefix("""
        ...
        |     %4 : () = llvm.br("loop_header0")
        |     %5 : () = llvm.label("loop_header0")
        |     %6 : () = llvm.phi([%3, %34], ["entry", "loop_exit1"])
        |     %7 : index = 2
        |     %8 : () = llvm.icmp("slt", %6, %7)
        |     %9 : () = llvm.cond_br(%8, "loop_body0", "loop_exit0")
        |     %10 : () = llvm.label("loop_body0")
        ...
        |     %12 : () = llvm.br("loop_header1")
        |     %13 : () = llvm.label("loop_header1")
        |     %14 : () = llvm.phi([%11, %30], ["loop_body0", "loop_body1"])
        |     %15 : index = 3
        |     %16 : () = llvm.icmp("slt", %14, %15)
        |     %17 : () = llvm.cond_br(%16, "loop_body1", "loop_exit1")
        |     %18 : () = llvm.label("loop_body1")
        ...
        |     %41 : () = llvm.phi([%38, %69], ["loop_exit0", "loop_exit3"])
        |     %42 : index = 2
        |     %43 : () = llvm.icmp("slt", %41, %42)
        |     %44 : () = llvm.cond_br(%43, "loop_body2", "loop_exit2")
        |     %45 : () = llvm.label("loop_body2")
        |     %46 : index = 0
        |     %47 : () = llvm.br("loop_header3")
        |     %48 : () = llvm.label("loop_header3")
        |     %49 : () = llvm.phi([%46, %65], ["loop_body2", "loop_body3"])
        |     %50 : index = 3
        |     %51 : () = llvm.icmp("slt", %49, %50)
        |     %52 : () = llvm.cond_br(%51, "loop_body3", "loop_exit3")
        ...
        |     %76 : () = llvm.phi([%73, %110], ["loop_exit2", "loop_exit5"])
        |     %77 : index = 3
        |     %78 : () = llvm.icmp("slt", %76, %77)
        |     %79 : () = llvm.cond_br(%78, "loop_body4", "loop_exit4")
        |     %80 : () = llvm.label("loop_body4")
        |     %81 : index = 0
        |     %82 : () = llvm.br("loop_header5")
        |     %83 : () = llvm.label("loop_header5")
        |     %84 : () = llvm.phi([%81, %106], ["loop_body4", "loop_body5"])
        |     %85 : index = 2
        |     %86 : () = llvm.icmp("slt", %84, %85)
        |     %87 : () = llvm.cond_br(%86, "loop_body5", "loop_exit5")
