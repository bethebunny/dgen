...
Walks into goto.label bodies to lower memory ops after control flow lowering.
...
def _extract_indices(pack: dgen.Value) -> list[dgen.Value]:
    """Extract individual index values from a PackOp."""
...
class MemoryToLLVM(Pass):
    ...
    def lower_dealloc(self, op: memory.DeallocOp, rewriter: Rewriter) -> bool:
        # Heap-allocated; no free for now.
