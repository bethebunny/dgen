...
_PIPELINE_IR = strip_prefix("""
    ...
    |             %5 : ndbuffer.NDBuffer<ndbuffer.Shape<1>([4]), number.Float64> = memory.heap_allocate<number.Float64>(4)
    ...
    |             %16 : ndbuffer.NDBuffer<ndbuffer.Shape<1>([4]), number.Float64> = memory.heap_allocate<number.Float64>(4)
...
def test_unfused_pipeline() -> None:
    ...
    module = asm.parse(
        ...
        |             %16 : ndbuffer.NDBuffer<ndbuffer.Shape<1>([2]), number.Float64> = memory.heap_allocate<number.Float64>(2)
