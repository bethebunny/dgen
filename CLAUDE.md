...
8. **Lower** memory → LLVM (`dgen/llvm/memory_to_llvm.py`)
9. **Codegen** → LLVM IR text → JIT via llvmlite (`dgen/llvm/codegen.py`, three-phase: see `docs/codegen.md`)
...
- `dgen/` — Core IR framework (dialect-independent)
  - `type.py`, `op.py`, `block.py`, `dialect.py` — Core IR types
  - `memory.py` — Typed memory buffer (`Memory[T]`)
  - `layout.py` — Memory layout descriptors for types
  - `trait.py` — Trait base class
  - `builtins.py` — Hand-written builtin ops (`ConstantOp`, `PackOp`, `pack`)
  - `asm/` — IR text formatting (`formatting.py`) and parsing (`parser.py`)
  - `ir/` — IR analysis (`traversal.py`, `verification.py`, `equivalence.py`, `diff.py`)
  - `passes/` — Pass framework (`pass_.py`, `compiler.py`, `staging.py`) and core passes
  - `llvm/` — LLVM backend (`codegen.py`, `algebra_to_llvm.py`, `builtin_to_llvm.py`, `memory_to_llvm.py`)
  - `spec/` — Dialect specification compiler (`parser.py`, `builder.py`, `importer.py`, `stubs.py`)
  - `dialects/` — Core dialect definitions (`.dgen` files and generated `.pyi` stubs)
...
Files marked `# GENERATED by dgen from X.dgen — do not edit.` are `.pyi` type stubs generated by `python -m dgen.spec`. **Never hand-edit generated files.** If generated output needs to change, fix the `.dgen` source or the generator (`dgen/spec/`). Committed generated files must always be exactly the current output of the generator — if regenerating a file would produce a diff, the committed version is wrong.
