...
def _resolve(type_of: dict[int, toy.TensorType], val: builtin.Value) -> toy.TensorType | None:
    return type_of.get(id(val))


def _infer_function(func: builtin.FuncOp, func_map: dict[str, builtin.FuncOp],
        ...
        if isinstance(op, builtin.ConstantOp) and isinstance(op.type, toy.TensorType):
            ...
            if all(t is not None for t in arg_types):
                ...
                if callee is not None:
                    ...
                    if isinstance(ret_op, builtin.ReturnOp) and ret_op.value is not None:
                        ret_type = _resolve(type_of, ret_op.value)
                        if ret_type is not None:
                            callee.func_type = toy.FunctionType(
                                inputs=list(arg_types), result=ret_type,
                            )
