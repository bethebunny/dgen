...
def test_constant_print():
    ...
    assert "ret void" in result, "Should return void"
...
def test_transpose():
    ...
    result = compile(source)
    assert "alloca f64, 6" in result, "Should have alloca for tensor"
...
def test_reshape_folds_away():
    ...
    result = compile(source)
    assert "alloca f64, 6" in result, "Should have single alloc"
    assert "call @print_memref" in result, "Should call print_memref"
...
def test_double_transpose_optimized():
    ...
    result = compile(source)
    assert "alloca f64, 6" in result, "Should have alloc for constant"
    assert "call @print_memref" in result, "Should call print_memref"
    assert "ret void" in result, "Should return void"
